...
const Orders = () => {
  ...
  const updateStatus = async (event, orderCode) => {
    const newStatus = event.target.value;
    const success = await updateOrderStatus(orderCode, newStatus, token);
              ...
              {orders.map((order, index) => (
                <tr key={index}>
                  ...
                  <td>
                    <select
                      className="form-control"
                      onChange={(event) => updateStatus(event, order.orderCode)}
